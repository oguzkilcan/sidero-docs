...
<Info>
    This is an experimental feature. It does not support Talos installations with custom built Linux kernel and custom built extensions.
</Info>
...
`omnictl cluster import` command have several important flags that will be discovered from the cluster or local environment if not provided. These flags can be seen below. Note that this list doesn't include all possible flags.
...
To check what will happen during the import process you can run `omnictl cluster import` with `--dry-run` flag:
...
$ omnictl cluster import \
  ...
  --initial-talos-version v1.11.2 \
  --initial-kubernetes-version v1.31.1
importing cluster "talos-default" to Omni
discovering Talos cluster state...
importing cluster "talos-default" with 8 nodes (1 control planes, 7 workers)
validating cluster status...
...
Note that the cluster has been imported but marked as `locked`. This is to prevent any changes being done by Omni to the imported cluster.
...
Cluster import command tries to detect if a Talos machine was configured using a boot asset from an Image Factory, hence connected to a Schematic. If so, it will try to ensure that Schematic with Omni's connected Image Factory.
...
If a custom Linux kernel or custom extensions were used while configuring the machine, they will be lost upon upgrade.
...
While importing a cluster to Omni, the idea is to leave the current state of the cluster as is and not to change any configuration on the nodes, unless it's absolutely necessary.
...
Certain machine config fields are not allowed to be set on Omni managed clusters. These fields are not included in the final config patch.
...
Reverting this step should be done manually and can be achieved by using the command below for each node:
```shell
PATCH=$(cat <<EOF
apiVersion: v1alpha1
